...
```bash
# Template
ssh -o ProxyJump=root@34.218.79.190 \
    -i ~/.ssh/rw-ssh-key \
    root@10.X.0.2
...
|---------|-------------|-----------|-------|
| Hypervisor | 172.31.46.94 | 34.218.79.190 | AWS a1.metal in us-west-2 |
...
scp -i ~/.ssh/rw-ssh-key localfile.txt root@34.218.79.190:/path/

# Copy file from hypervisor
scp -i ~/.ssh/rw-ssh-key root@34.218.79.190:/path/file.txt ./
```
...
```bash
# Copy to VM (two-step)
scp -i ~/.ssh/rw-ssh-key localfile.txt root@34.218.79.190:/tmp/
ssh hypervisor 'scp -i /root/.ssh/id_ed25519 /tmp/localfile.txt root@10.1.0.2:/path/'

# Or with ProxyJump
scp -o ProxyJump=root@34.218.79.190 -i ~/.ssh/rw-ssh-key \
...
ssh -L 8080:10.1.0.2:8080 -i ~/.ssh/rw-ssh-key root@34.218.79.190
...
ssh -L 3389:10.1.0.2:3389 -i ~/.ssh/rw-ssh-key root@34.218.79.190
